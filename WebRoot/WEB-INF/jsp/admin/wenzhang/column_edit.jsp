...
<HTML>
	...
	<body>
		...
		<form id="editForm" data-parent=""  name="Form1" action="${pageContext.request.contextPath}/admin/article/column_add.action" method="post" enctype="multipart/form-data">
			&nbsp;
			<table cellSpacing="1" cellPadding="5" width="100%" align="center" bgColor="#eeeeee" style="border: 1px solid #8ba7e3" border="0">
				...
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff" colspan="3">
						<input type="text" name="colName" value="${item.colName}" id="userAction_save_do_logonName" class="bg"/>
					</td>
				</tr>
				...
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff" colspan="3">
						...
						<input type="button" value="预览" onclick="window.open('${pageContext.request.contextPath}/${item.image }');">
					</td>
				</tr>
	...
	<c:if test="${!empty column}">
		<form id="editForm" data-curr="${column.id}" data-parent="${column.parent.id}" name="Form1" action="${pageContext.request.contextPath}/admin/article/column_update.action" method="post" enctype="multipart/form-data">
			&nbsp;
			<table cellSpacing="1" cellPadding="5" width="100%" align="center" bgColor="#eeeeee" style="border: 1px solid #8ba7e3" border="0">
				<tr>
					<td class="ta_01" align="center" bgColor="#afd1f3" colSpan="4"
	...
	var select = $("#columns select:last");
		...
		if(!select.val())select = select.prev();
		
		
		$("#editForm input[name='parent.id']").val(select.val());
		
		var goback = $("#editForm input[name='goback']");
	if(goback.length>0)
	goback.val("${pageContext.request.contextPath}/admin/article/column_query.action");
	else $("#editForm").append('<input type="hidden" name="goback" value="${pageContext.request.contextPath}/admin/article/column_query.action"/>');
	...
	var model = {};
		...
		$(function(){
					...
					if(!pid)
					printStart({curr:null,list:rootlist()});
					//判断新增
					if(!currid)return;
