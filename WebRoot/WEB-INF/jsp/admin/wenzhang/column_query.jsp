				...
				<tr>
						<td class="ta_01" align="left" bgColor="#afd1f3">
							<button type="button" id="add" name="add" value="添加" class="button_add"
							 onclick="window.location.href='${pageContext.request.contextPath}/admin/article/column_add.action?action=index'">
&#28155;&#21152;
						</td>
...
/* ----------------------------- */
	...
	if(!ids)return; 
	App.del("${pageContext.request.contextPath}/admin/article/column_delete.action",ids);
	
	}
	
	
	function onEdit(id){
		getRequest("${pageContext.request.contextPath}/admin/article/column_update.action",{action:"index",id:id});
	}
	
	function mdel(id){
		App.del("${pageContext.request.contextPath}/admin/article/column_delete.action",[id]);
	}
			
	function onAdd(parent){
	getRequest("${pageContext.request.contextPath}/admin/article/column_add.action",{action:"index",parent:parent});
...
<tr data-id="{{val.id}}" {{if val.parent}} data-parent="{{val.parent.id}}"{{/if}} onmouseover="this.style.backgroundColor = 'white'" onmouseout="this.style.backgroundColor = '#F5FAFE';">
										...
										<input data-id="{{val.id}}" onchange="onItemChange()" class="select item_check" type="checkbox">						
												...
												<td align="center" style="HEIGHT: 22px">
														...
														<td align="center" style="HEIGHT: 22px">
														{{if val.image}}
															 <img src="${pageContext.request.contextPath}/{{val.image}}">
														{{/if}}
